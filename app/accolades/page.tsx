...
export default async function AccoladesPage() {
  ...
  const awards = data.accolades.filter((item) => item.type === 'award');
  const records = data.accolades.filter((item) => item.type === 'record');

  return (
    <>
      <PageIntro eyebrow="League history" title="The names that remain." description="Season awards and record performances preserved across the official PBL archive." />
      <div className="site-shell space-y-14 py-12 sm:py-16">
        {data.accolades.length === 0 ? (
          ...
          <>
            <AccoladeSection title="Season honors" description="Awards selected by the league office and verified voting panel." items={awards} icon={Award} teams={data.teams} />
          ...
          const card = (
            <article className="lift relative isolate overflow-hidden rounded-[1.5rem] border border-[var(--line)] bg-[var(--surface)] p-5 sm:p-6">
              ...
              <div className="flex items-start gap-4 pr-14">
                {team ? <TeamLogo team={team} size="md" /> : <span className="grid h-12 w-12 place-items-center rounded-2xl bg-[var(--surface-soft)]"><Icon className="h-5 w-5 text-[var(--ink-faint)]" /></span>}
                <div><p className="text-[0.65rem] font-black uppercase tracking-[0.13em] text-[var(--orange-soft)]">{item.title}</p><h3 className="mt-2 text-xl font-black tracking-[-0.035em]">{item.recipient}</h3></div>
              </div>
              <p className="mt-5 border-t border-[var(--line)] pt-4 text-sm leading-6 text-[var(--ink-soft)]">{item.description}</p>
            </article>
          );
          return team ? <Link key={item.id} href={`/teams/${team.slug}`}>{card}</Link> : <div key={item.id}>{card}</div>;
        })}
      </div>
    </section>
  );
