...
export function RaceHome({ data }: { data: SiteData }) {
  ...
  const leaders = [...data.players].sort((a, b) => b.stats.pointsPerGame - a.stats.pointsPerGame).slice(0, 3);
  const leaderTeam = data.teams.find((team) => team.id === rankings[0]?.id);
  return (
    <>
      <section className="relative isolate overflow-hidden border-b border-[var(--line)]">
        ...
        <div className="site-shell relative grid min-h-[calc(100vh-4.5rem)] gap-10 py-16 lg:grid-cols-[1.08fr_.92fr] lg:items-center lg:py-24">
          <motion.div initial={{ opacity: 0, y: 26 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .65, ease: [0.2, .8, .2, 1] }}>
            ...
            <div className="mt-9 flex flex-wrap gap-3"><Link href="/games" className="inline-flex h-12 items-center gap-2 bg-[var(--orange)] px-6 text-xs font-black uppercase italic tracking-[.12em] text-black transition hover:translate-x-1">{t('watchLive')} <ChevronRight className="h-4 w-4" /></Link><Link href="/standings" className="inline-flex h-12 items-center gap-2 border border-[var(--line-strong)] bg-black/20 px-6 text-xs font-black uppercase italic tracking-[.12em] transition hover:border-[var(--orange)]">{t('viewStandings')}</Link></div>
            <div className="mt-14 grid max-w-xl grid-cols-3 divide-x divide-[var(--line)] border-y border-[var(--line)]"><Telemetry label={t('season', { season: data.season.name })} value={data.teams.length} /><Telemetry label="TEAMS" value={data.players.length} /><Telemetry label="GAMES" value={data.games.length} /></div>
          </motion.div>
