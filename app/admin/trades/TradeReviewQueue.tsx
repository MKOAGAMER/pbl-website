        ...
        <div className="space-y-4">
          {pending.length === 0 ? <EmptyQueue /> : pending.map((trade) => (
            <article key={trade.id} className="rounded-[1.4rem] border border-[var(--line)] bg-[var(--surface)] p-5">
              <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[0.6rem] font-black uppercase tracking-[0.12em] text-[var(--ink-faint)]">{trade.requestKind === 'acquire' ? 'ซื้อเข้าทีม' : trade.requestKind === 'release' ? 'ขายออกจากทีม' : 'ย้ายทีม'} · {new Intl.DateTimeFormat('th-TH', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(trade.requestedAt))}</p><h3 className="mt-2 text-xl font-black">{trade.playerName}</h3></div><Clock3 className="h-5 w-5 text-amber-300" /></div>
